...
export default function App() {
  ...
  Modal: {
    name: 'Modal',
    description:
      '基于 antd Modal + Form 封装的弹窗表单组件（导出名为 ModalList）。支持动态表单项、加载状态、提交/取消回调。',
    ...
    props: [
      ...
      },
      {
        ...
        required: true,
        description:
          '弹窗配置对象，包含 title(标题)、width(宽度,默认520)、layout(表单布局)、initialValues(初始值)、listData(表单项数组)',
      },
      {
        name: 'data.listData',
        ...
        required: true,
        description:
          '表单项列表。label:标签名, name:字段名, render:表单控件(ReactNode), rules:校验规则, style:样式',
      },
      {
        name: 'onFinish',
...
export default function App() {
  ...
  TextEllipsis: {
    name: 'TextEllipsis',
    description:
      '文本超出容器宽度时自动截断并显示 Tooltip 提示完整内容。内容为 undefined/null 时显示 "------"。',
...
export default function App() {
  ...
  VirtualTable: {
    name: 'VirtualTable',
    description:
      '基于 antd Table 封装的表格组件，支持分页配置和透传所有 antd Table 属性。',
